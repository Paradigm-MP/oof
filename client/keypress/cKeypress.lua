...
function KeyPress:__init()
    self.check_keys = {}

    self:CheckForPressedKeys()
end

-- FYI - keypresses don't work when a UI has focus

--[[
    Subscribe KeyPress manager to a key so you can receive events about when it is pressed (KeyUp and KeyDown)
    (ref counted)
]]
function KeyPress:Subscribe(key)
    self.check_keys[key] = self.check_keys[key] and self.check_keys[key] + 1 or 1
end

--[[
    Unsubscribe a key from keypress so you don't get events anymore
]]
function KeyPress:Unsubscribe(key)
    if self.check_keys[key] then
        self.check_keys[key] = self.check_keys[key] - 1
        if self.check_keys[key] <= 0 then
            self.check_keys[key] = nil
        end
    end
end
...
-- FiveM list of controls: https://github.com/citizenfx/fivem-docs/blob/master/content/docs/game-references/controls.md
if IsRedM then
    Control = 
    {
...
-- FiveM list of controls: https://github.com/citizenfx/fivem-docs/blob/master/content/docs/game-references/controls.md
if IsFiveM then
    -- List of all controls from https://github.com/crosire/scripthookvdotnet/blob/master/source/scripting_v2/Game/Controls.hpp
    Control = 
    {
        NextCamera = 0,
        LookLeftRight = 1,
